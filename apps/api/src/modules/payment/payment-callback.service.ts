...
export class PaymentCallbackService {
  ...
  async process(result: NormalizedPaymentResult) {
    ...
    const payment = await paymentRecordModel.findFirst({
      where: { providerOrderNo: result.providerOrderNo },
    });
    ...
    if (order.userId !== String(payment['userId']))
      throw new BadRequestException('支付记录与订单用户不一致');
    if (order.amountCents !== result.amount)
      throw new BadRequestException('回调金额与订单金额不一致');

    const dup = await paymentRecordModel.findMany({
      where: { providerTradeNo: result.providerTradeNo },
    });
