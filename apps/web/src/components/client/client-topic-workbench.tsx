...
type TopicCandidate = { id: string; title: string; generationBatch: number; rationale?: string };
type BootstrapTask = { id: string; title: string; status: string };
type TaskListItem = { id: string; title: string; status: string; createdAt: string };
type TaskListResp = { items: TaskListItem[] };
...
export function ClientTopicWorkbench({ taskId }: { taskId?: string }) {
  ...
  const loadMyTasks = useCallback(async () => {
    try { setTaskLoading(true); setTaskError(null);
      const data = await clientHttp.get<TaskListResp>('/tasks/me', { page: 1, pageSize: 10 });
      setTasks(data.items || []);
    } catch { setTaskError('加载我的任务失败'); }
  ...
  const emptyReason = useMemo(() => {
    if (!activeTaskId) return '请先创建任务、或从“我的任务”选择任务、或手动输入 taskId。';
  ...
  const loadCandidates = useCallback(async () => {
    ...
    try { setLoading(true); setError(null); setItems(await clientHttp.get(`/tasks/${activeTaskId}/topics/latest`)); }
    catch { setError('加载题目候选失败，请稍后重试'); }
    finally { setLoading(false); }
  }, [activeTaskId]);
  useEffect(() => { void loadCandidates(); }, [loadCandidates]);

  const handleBootstrap = async (e: FormEvent<HTMLFormElement>) => {
    e.preventDefault(); if (bootstrapping) return;
    try {
      ...
      setActiveTaskId(task.id); window.history.replaceState(null, '', `/tasks?taskId=${encodeURIComponent(task.id)}`);
      await loadMyTasks();
    } catch { setError('创建任务失败，请稍后重试'); }
    finally { setBootstrapping(false); }
  };

  const handleGenerate = async (e: FormEvent<HTMLFormElement>) => {
    e.preventDefault(); if (submitting || !activeTaskId) return;
    if (count < 1 || count > 10) return setError('候选数量必须在 1 到 10 之间');
    try { setSubmitting(true); setError(null); await clientHttp.post(`/tasks/${activeTaskId}/topics/generate`, { count, additionalContext: additionalContext || undefined, preferredStyle: preferredStyle || undefined }); await loadCandidates(); }
  ...
  return <section className="space-y-4">
    ...
    <div className="rounded-xl border border-slate-200 bg-white p-4">
      <h2 className="mb-2 font-medium">我的任务</h2>
      {taskLoading ? <p className="text-sm text-slate-600">加载中...</p> : null}
      {taskError ? <p className="text-sm text-red-600">{taskError}</p> : null}
      {!taskLoading && !taskError && tasks.length === 0 ? <p className="text-sm text-slate-600">暂无任务，请先创建。</p> : null}
      <ul className="space-y-2">{tasks.map((t) => <li key={t.id}><button type="button" onClick={() => { setActiveTaskId(t.id); window.history.replaceState(null, '', `/tasks?taskId=${encodeURIComponent(t.id)}`); }} className="w-full rounded border border-slate-300 px-3 py-2 text-left text-sm hover:bg-slate-50">{t.title || t.id} <span className="text-xs text-slate-500">({t.status})</span></button></li>)}</ul>
    </div>
